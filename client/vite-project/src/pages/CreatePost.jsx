...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch("https://red-4ax1.onrender.com/api/v1/dalle/", {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setIsLoading(true);
      try {
        const response = await fetch("https://red-4ax1.onrender.com/api/v1/post", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
  ...
  return (
    <section className="max-w-7xl mx-auto">
      ...
      <div>
        ...
        <p className="mt-2 text-[#0e1612]  text-center text-[16px] max-w-[100%] bg-[#36ffa8] p-3 rounded-md ">Generate an imaginative image through DALL-E AI and share it with the community</p>
      </div>

      <div className="bg-green-100 rounded">
      <form className="mt-16 max-w-3xl bg-[#a7f1cfef] p-3 rounded" onSubmit={handleSubmit}>
        ...
        <div className="mt-5 flex gap-5">
          <button
            ...
            className="text-[#192721] bg-[#36ffa8] font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center border-2 border-[#fafcfc]"
          >
        ...
        <p className="mt-2 text-[#121213] text-[15px]">Once you have created the image you want, you can share it with others in the community</p>
          <button
            type="submit"
            className="mt-3 text-white bg-[#23b1d4] border-2 border-white font-medium rounded-md text-sm w-full sm:w-auto px-5 py-2.5 text-center"
          >
